...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    app.get('/api/meetings', async (c) => {
        try {
            ...
            return c.json({ success: false, error: 'Failed to retrieve meetings' }, { status: 500 });
        }
    });
    app.post('/api/meetings', async (c) => {
        try {
            ...
            const controller = getAppController(c.env);
            const newMeeting = await controller.addMeeting(body);
            return c.json({ success: true, data: newMeeting });
        } catch (error) {
            console.error('Failed to save meeting:', error);
            return c.json({ success: false, error: 'Failed to save meeting' }, { status: 500 });
        }
    });
